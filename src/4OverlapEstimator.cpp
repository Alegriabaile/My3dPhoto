...
namespace m3d
{
    ...
    void OverlapEstimator::CalcInitialRt2d2d(Edge &edge, size_t index)
    {
        ...
        //        Mat mask;
        Mat essential_matrix = findEssentialMat(pts1, pts2, intrinsicK, RANSAC, 0.999, 1.0, edge.essentialMask1);
        ...
        Mat R, t;
        edge.essentialMask2 = edge.essentialMask1.clone();
        recoverPose(essential_matrix, pts1, pts2, intrinsicK, R, t, edge.essentialMask2);
        ...
        std::vector<cv::KeyPoint> srcPtsInlier, dstPtsInlier;
        for(size_t k = 0; k < edge.essentialMask1.rows; ++k)
        {
            if(edge.essentialMask2.at<uchar>(k))
            {
                srcPtsInlier.push_back(edge.srcPts[k]);
                dstPtsInlier.push_back(edge.dstPts[k]);
            }
        }
        edge.srcPts = srcPtsInlier;
        edge.dstPts = dstPtsInlier;
    }
